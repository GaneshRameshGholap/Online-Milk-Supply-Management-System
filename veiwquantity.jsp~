...
<%
out.println("<br><br>");
float total=(Float)session.getAttribute("quantity");
out.println("<h3><center>Total Quantity="+total+"</center></h3>");
%>
...
try
{
	...
	String dd=(String)session.getAttribute("date");
	...
	else
	{
		...
		out.println("<center><table border=2><tr><th>Customer Name</th><th>Quantity</th></tr>");
		res=stmt.executeQuery("select cname,qty from updated where changedate='"+dd+"' and eid="+id);
		while(res.next())
		{
                       String str=res.getString("cname");
                       float f=res.getFloat("qty");
			out.println("<tr><td>"+str+"</td><td>"+f+"</td></tr>");
