...
try
{
	...
	{         
                  ...
                  rss=stmt.executeQuery("select count(*) from customer");
                  if(rss.next())
                  {
                      len=Integer.parseInt(rss.getString(1));
                     
                
                      rs=stmt.executeQuery("select cid from combine group by  cid");
		      while(rs.next())
		      {
			    cid=rs.getInt("cid");
                    ...
                    pstmt=con.prepareStatement("Insert into bill values(default,?,?,?,?,?,?,?)");
	              for(i=0;i<=(len-1);i++)
                      {
                      
                         sum=0;
                         res=stmt.executeQuery("select  sum(milk_qty) from combine where cid="+arr[i]+" and date between '"+from+"' and '"+to+"'");
			 if(res.next())
                         {
			 m=res.getString(1);
			 ...
			 res1=stmt.executeQuery("select rate from customer where cid="+arr[i]);
                         res1.next();
			 no2=res1.getFloat("rate");
			 sum=sum*no2;
			 ...
			 out.println("<h1>"+no1+"</h1>");
                         out.println("<h1>"+no2+"</h1>");
